...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    lv_obj_set_style_pad_all(msg_bubble, lvgl_theme->spacing(6), 0);  // 增加内边距，提高可读性
    ...
    lv_coord_t max_width = LV_HOR_RES * 80 / 100;  // 80% of screen width，减少最大宽度使对话框更美观
    lv_coord_t min_width = 50;  
    ...
    // Set bubble width with padding
    lv_obj_set_width(msg_bubble, LV_SIZE_CONTENT);
    lv_obj_set_height(msg_bubble, LV_SIZE_CONTENT);
    
    // For bubble width calculation, add padding to text width
    bubble_width += lvgl_theme->spacing(12);  // Add padding
    lv_obj_set_style_max_width(msg_bubble, bubble_width, 0);  // Set max width

    // Set alignment and style based on message role
    if (strcmp(role, "user") == 0) {
        // User messages are right-aligned with green background (WeChat style)
        lv_obj_set_style_bg_color(msg_bubble, lv_color_hex(0x07C160), 0);  // WeChat green color
        lv_obj_set_style_bg_opa(msg_bubble, LV_OPA_100, 0);  // Full opacity
        
        // WeChat style asymmetric radius (larger on top-left and bottom-right)
        lv_obj_set_style_radius(msg_bubble, 18, 0);
        lv_obj_set_style_radius_bottom_right(msg_bubble, 4, 0);
        ...
        // Add subtle shadow
        lv_obj_set_style_shadow_color(msg_bubble, lv_color_hex(0x000000), 0);
        lv_obj_set_style_shadow_opa(msg_bubble, LV_OPA_10, 0);
        lv_obj_set_style_shadow_width(msg_bubble, 2, 0);
        lv_obj_set_style_shadow_offset_x(msg_bubble, 1, 0);
        lv_obj_set_style_shadow_offset_y(msg_bubble, 1, 0);
        
        // Don't grow
        lv_obj_set_style_flex_grow(msg_bubble, 0, 0);
    } else if (strcmp(role, "assistant") == 0) {
        // Assistant messages are left-aligned with white background (WeChat style)
        lv_obj_set_style_bg_color(msg_bubble, lv_color_white(), 0);  // White background
        lv_obj_set_style_bg_opa(msg_bubble, LV_OPA_100, 0);  // Full opacity
        
        // WeChat style asymmetric radius (larger on top-right and bottom-left)
        lv_obj_set_style_radius(msg_bubble, 18, 0);
        lv_obj_set_style_radius_bottom_left(msg_bubble, 4, 0);
        
        // Dark gray text for assistant messages
        lv_obj_set_style_text_color(msg_text, lv_color_hex(0x333333), 0);
        
        // Set custom attribute to mark bubble type
        lv_obj_set_user_data(msg_bubble, (void*)"assistant");
        
        // Light gray border
        lv_obj_set_style_border_color(msg_bubble, lv_color_hex(0xE5E5E5), 0);
        ...
        lv_obj_set_style_bg_color(msg_bubble, lv_color_hex(0xF2F2F2), 0);  // Light gray background
        lv_obj_set_style_bg_opa(msg_bubble, LV_OPA_100, 0);  // Full opacity
        
        // Symmetric radius for system messages
        lv_obj_set_style_radius(msg_bubble, 12, 0);
        
        // Gray text for system messages
        lv_obj_set_style_text_color(msg_text, lv_color_hex(0x999999), 0);
    ...
    // Create a full-width container for all messages
    ...
    // WeChat style spacing between messages
    lv_obj_set_style_pad_top(container, lvgl_theme->spacing(3), 0);
    lv_obj_set_style_pad_bottom(container, lvgl_theme->spacing(3), 0);
    
    // Move the message bubble into this container
    lv_obj_set_parent(msg_bubble, container);
    
    if (strcmp(role, "user") == 0) {
        // Right align user messages with WeChat style margin
        lv_obj_align(msg_bubble, LV_ALIGN_RIGHT_MID, -20, 0);
    } else if (strcmp(role, "system") == 0) {
        // Center align system messages
        lv_obj_align(msg_bubble, LV_ALIGN_CENTER, 0, 0);
    } else {
        // Left align assistant messages with WeChat style margin
        lv_obj_align(msg_bubble, LV_ALIGN_LEFT_MID, 20, 0);
    }
